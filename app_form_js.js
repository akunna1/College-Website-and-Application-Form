...
document.addEventListener('DOMContentLoaded', function () { // DOMContentLoaded is to get references to elements after the page has fully loaded.
...
// 18) Function to check if all required fields are filled and for form validity before one is allowed to submit
// Enabling Submit Button Section
// Defining constants for the function
...
// Event listeners for form inputs --> checking validity
...
// Use AWS SDK to upload to Amazon S3 (ensure you've loaded the SDK script)
// Example: AWS Amplify Storage or AWS SDK for JavaScript
  
// Once uploaded, trigger a function to send an email using Rails Action Mailer


// 20) Payment Section (might have to move this to server side code)
